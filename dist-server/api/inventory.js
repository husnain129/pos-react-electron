...
router.get("/product/:productId", async (req, res) => {
    if (!req.params.productId) {
        ...
        try {
            const result = await config_1.default.query(`SELECT p.id as _id, p.product_name as name, p.price, p.cost_price, p.barcode,
        ...
        FROM products p 
        WHERE p.id = $1`, [parseInt(req.params.productId)]);
...
router.post("/product", async (req, res) => {
    try {
        const categoryResult = await config_1.default.query("SELECT name FROM categories WHERE id = $1", [parseInt(req.body.category)]);
        ...
        if (req.body.id === "" || !req.body.id) {
            ...
            if (req.body.institute_id) {
                ...
                const instituteResult = await config_1.default.query("SELECT name, zone, district FROM institutes WHERE id = $1", [parseInt(req.body.institute_id)]);
            ...
            const result = await config_1.default.query(`INSERT INTO products (product_name, price, cost_price, product_category, category_id, quantity, product_specifications, institute_id, zone, district, institute_name) 
            ...
            console.log("Generated barcode:", barcode, "for product ID:", newProductId);
            // Update the product with generated barcode
            await config_1.default.query(`UPDATE products SET barcode = $1 WHERE id = $2`, [
                barcode,
                newProductId,
            ]);
            // Fetch and return complete product data
            const productResult = await config_1.default.query(`SELECT p.id as _id, p.product_name as name, p.price, p.cost_price, p.barcode,
        COALESCE(p.category_id, 0) as category, p.quantity, p.product_specifications as stock, 
        p.zone, p.district, p.institute_name, p.institute_id, p.product_category,
        p.created_at, p.updated_at
        FROM products p WHERE p.id = $1`, [newProductId]);
            ...
            await config_1.default.query(`UPDATE products SET product_name = $1, price = $2, cost_price = $3, product_category = $4, category_id = $5, quantity = $6, product_specifications = $7, institute_id = $8, zone = $9, district = $10, institute_name = $11, updated_at = CURRENT_TIMESTAMP WHERE id = $12`, [
                req.body.name,
                req.body.price,
                req.body.cost_price || 0,
                categoryName,
...
router.delete("/product/:productId", async (req, res) => {
    try {
        await config_1.default.query("DELETE FROM products WHERE id = $1", [
...
async function decrementInventory(products) {
    for (const transactionProduct of products) {
        try {
            const result = await config_1.default.query("SELECT id, quantity FROM products WHERE id = $1", [parseInt(transactionProduct.id)]);
            const product = result.rows[0];
            if (product && product.quantity) {
                let updatedQuantity = parseInt(product.quantity) - parseInt(transactionProduct.quantity);
                await config_1.default.query("UPDATE products SET quantity = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2", [updatedQuantity, parseInt(product.id)]);
            }
        }
        catch (err) {
